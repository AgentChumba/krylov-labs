...
void Interpreter::executeLines(const std::vector<std::string> &lines,
                               int &index, int level) {
...
void Interpreter::handleLoop(const std::string &line,
  ...
  if (!isAllowedVariable(variable)) {
    result += "Ошибка: недопустимое имя переменной '" + variable +
              "'. Используйте только resistor или cuvette.\r\n";
    return;
  }
  ...
  result += "Начало цикла " + std::to_string(loop.level) +
            " уровня: " + loop.variable + " от " + std::to_string(min) +
...
void Interpreter::handleAssignment(const std::string &line) {
  ...
  result +=
      "Присваивание: " + variable + " = " + std::to_string(value) + "\r\n";
}
...
void Interpreter::handleSaveAll() { result += "Сохранение результатов\r\n"; }
