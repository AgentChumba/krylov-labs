...
Tab1Widget::Tab1Widget(QWidget *parent) : QWidget(parent) { setupUi(); }
...
void Tab1Widget::setupUi() {
  ...
  connect(browseButton, &QPushButton::clicked, this,
          &Tab1Widget::browseLibraryPath);
  connect(loadLibraryButton, &QPushButton::clicked, this,
          &Tab1Widget::loadLibrary);
  connect(invokeFunctionButton, &QPushButton::clicked, this,
          &Tab1Widget::invokeFunction);
}
...
void Tab1Widget::browseLibraryPath() {
  ...
  QString filePath =
      QFileDialog::getOpenFileName(this, "Выберите библиотеку", "", filter);
  if (!filePath.isEmpty()) {
...
void Tab1Widget::loadLibrary() {
  ...
  QRegularExpression regex(
      R"(\d+\s+\d+\s+[0-9a-fA-F]+\s+([a-zA-Z_][\w@]*)\s*(=.*)?)");
  ...
  QRegularExpression regex(R"(^[0-9a-fA-F]+ T (\w+)$)",
    ...
    if (match.hasMatch()) {
      ...
      if (filtProcess.waitForFinished()) {
        ...
        QString functionName =
            (parenIndex != -1) ? demangledName.left(parenIndex) : demangledName;
  ...
  if (!found)
    resultEdit->setText("Функции не найдены!");
  else
    resultEdit->setText("Библиотека загружена!");
#endif
}
...
#ifdef Q_OS_WIN
  func = reinterpret_cast<FunctionPointer>(
      GetProcAddress(libraryHandle, functionName.toStdString().c_str()));
#else
  func = reinterpret_cast<FunctionPointer>(
      dlsym(libraryHandle, mangledName.toStdString().c_str()));
#endif
